...
class ModalityEncoder(nn.Module):
    def __init__(self, in_channels=1, base_channels=32):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv3d(in_channels, base_channels, kernel_size=7, stride=1, padding=3),  # 2×
            nn.GroupNorm(8, base_channels),
            nn.ReLU(inplace=True)
        )
        self.pool = nn.MaxPool3d(kernel_size=2, stride=2, padding=0)  # 4×
        self.conv2 = nn.Sequential(
            nn.Conv3d(base_channels, 4 * base_channels, kernel_size=3, stride=2, padding=1),  # 4×
            nn.GroupNorm(8, 4 * base_channels),
            nn.ReLU(inplace=True),
        )
        self.tsmamba = TSMamba(4 * base_channels)

    def forward(self, x):
        x = self.conv1(x)  # [B, C, H//2, W//2, D//2]
        x = self.pool(x)
        x = self.conv2(x)  # [B, C, H//4, W//4, D//4]
        x = self.tsmamba(x)
        return x


class MambaLayer(nn.Module):
    def __init__(self, dim, d_state=16, d_conv=4, expand=2, num_slices=4):
        super().__init__()
        self.dim = dim
        self.mamba = Mamba(
            d_model=dim,
    ...
    def forward(self, x, skip=True):
        B, C = x.shape[:2]
        x_skip = x
        assert C == self.dim, f"set dim={self.dim}, but got C={C}"
...
class MultiFusion(nn.Module):
    def __init__(self, in_channels, out_channels, reduction=8):
        ...
        self.conv = nn.Sequential(
            nn.Conv3d(in_channels, out_channels // 4, kernel_size=1, bias=False),
            nn.GroupNorm(8, out_channels // 4),
            nn.ReLU(inplace=True),
            nn.Conv3d(out_channels // 4, out_channels, kernel_size=3, padding=1, bias=False),
    ...
    def forward(self, x):
        # x: [B, 4C, D, H, W]
        b, _, _, _, _ = x.shape
        feat = self.conv(x)  # [B, C, D, H, W]
        y = self.pool(x).view(b, -1)        # [B, 4C]
        y = self.attention(y).view(b, -1, 1, 1, 1)  # [B, C, 1, 1, 1]
        return feat * y
...
class MProtoNet(nn.Module):
    def __init__(self, in_size=(4, 240, 240, 155), out_size=2, features='resnet50_ri', n_layers=7,
                 prototype_shape=(20, 128, 1, 1, 1), init_weights=True, f_dist='l2', mamba_dim=64,
        ...
        self.mamba_score = 0.0
        if p_mode >= 6:
            encoder_dim = self.mamba_dim // 4
            self.encoders = nn.ModuleList(
                [
                    ModalityEncoder(in_channels=1, base_channels=encoder_dim)
                    for _ in range(4)
                ]
            )
            """
            ...
            # """
            self.fuse = MultiFusion(4 * encoder_dim, add_ons_channels)
            layer_filter_sizes, layer_strides, layer_paddings = conv_info(self.encoders[0])
    ...
    def conv_features(self, x, modality_mask=None):

        if self.p_mode >= 6:
            t1, t1ce, t2, flair = torch.split(x, 1, dim=1)
            # modality-specific encoder
            x_t1 = self.encoders[0](t1)
            x_t1ce = self.encoders[1](t1ce)
            x_t2 = self.encoders[2](t2)
            x_flair = self.encoders[3](flair)
            # mask
            B, C, H, W, D = x_t1.shape
            stack_input = torch.stack([x_t1, x_t1ce, x_t2, x_flair], dim=-1) # B, C, H, W, D, 4
            if modality_mask is not None:  # modality missing mask
                modality_mask = modality_mask.view(-1, 1, 1, 1, 1, 4)  # B, 1, 1, 1, 1, 4
                stack_input = stack_input * modality_mask


            """
            stack_input_r = torch.flip(stack_input, dims=[-2, -3, -4])  # B, C, H, W, D, 4, reverse the HWD
            # global forward
            global_mamba_f = self.mambas[0](stack_input) # B, C, H, W, D, 4
            global_t1_f, global_t1ce_f, global_t2_f, global_flair_f = global_mamba_f.chunk(4, dim=-1)
            # global reverse
            global_mamba_r = self.mambas[1](stack_input_r) # B, C, H, W, D, 4
            global_t1_r, global_t1ce_r, global_t2_r, global_flair_r = global_mamba_r.chunk(4, dim=-1)
            # reshape: pixel first
            stack_input = stack_input.permute(0, 1, 5, 2, 3, 4) # B, C, 4, H, W, D
            stack_input_r = stack_input_r.permute(0, 1, 5, 2, 3, 4) # B, C, 4, H, W, D
            # local forward (pixel-wise)
            local_mamba_f = self.mambas[2](stack_input) # B, C, 4, H, W, D
            local_t1_f, local_t1ce_f, local_t2_f, local_flair_f = local_mamba_f.chunk(4, dim=2)
            # local reverse (pixel-wise)
            local_mamba_r = self.mambas[3](stack_input_r)  # B, C, 4, H, W, D
            local_t1_r, local_t1ce_r, local_t2_r, local_flair_r = local_mamba_r.chunk(4, dim=2)
            # fuse local and global
            x_t1 = (global_t1_f.squeeze(-1) + global_t1_r.squeeze(-1) + local_t1_f.squeeze(-4) + local_t1_r.squeeze(-4)) / 4
            x_t1ce = (global_t1ce_f.squeeze(-1) + global_t1ce_r.squeeze(-1) + local_t1ce_f.squeeze(-4) + local_t1ce_r.squeeze(-4)) / 4
            x_t2 = (global_t2_f.squeeze(-1) + global_t2_r.squeeze(-1) + local_t2_f.squeeze(-4) + local_t2_r.squeeze(-4)) / 4
            x_flair = (global_flair_f.squeeze(-1) + global_flair_r.squeeze(-1) + local_flair_f.squeeze(-4) + local_flair_r.squeeze(-4)) / 4
            # """
            ...
            global_mamba_r = self.mambas[1](stack_input_r) # B, C, H, W, D, 4
            global_t1_r, global_t1ce_r, global_t2_r, global_flair_r = global_mamba_r.chunk(4, dim=-1)
            x_t1 = (global_t1_f.squeeze(-1) + global_t1_r.squeeze(-1)) / 2
            x_t1ce = (global_t1ce_f.squeeze(-1) + global_t1ce_r.squeeze(-1)) / 2
            x_t2 = (global_t2_f.squeeze(-1) + global_t2_r.squeeze(-1)) / 2
            x_flair = (global_flair_f.squeeze(-1) + global_flair_r.squeeze(-1)) / 2
            # """
            ...
            stack_input_r = stack_input_r.permute(0, 1, 5, 2, 3, 4) # B, C, 4, H, W, D
            # local forward (pixel-wise)
            local_mamba_f = self.mambas[0](stack_input) # B, C, 4, H, W, D
            local_t1_f, local_t1ce_f, local_t2_f, local_flair_f = local_mamba_f.chunk(4, dim=2)
            # local reverse (pixel-wise)
            local_mamba_r = self.mambas[1](stack_input_r)  # B, C, 4, H, W, D
            local_t1_r, local_t1ce_r, local_t2_r, local_flair_r = local_mamba_r.chunk(4, dim=2)
            x_t1 = (local_t1_f.squeeze(-4) + local_t1_r.squeeze(-4)) / 2
            x_t1ce = (local_t1ce_f.squeeze(-4) + local_t1ce_r.squeeze(-4)) / 2
            x_t2 = (local_t2_f.squeeze(-4) + local_t2_r.squeeze(-4)) / 2
            x_flair = (local_flair_f.squeeze(-4) + local_flair_r.squeeze(-4)) / 2
            # """
            ...
            # local or global
            if modality_mask is not None:  # modality missing mask
                x_fuse = x_fuse * modality_mask # modality missing mask
                x_fuse = x_fuse.sum(dim=-1) / modality_mask.sum(dim=-1).clamp(min=1).view(-1, 1, 1, 1, 1)
            else:
                x_fuse = x_fuse.sum(dim=-1)
            # """


            # """
            # no mambas module
            x_t1, x_t1ce, x_t2, x_flair = stack_input.chunk(4, dim=-1)
            x_t1, x_t1ce, x_t2, x_flair = x_t1.squeeze(-1), x_t1ce.squeeze(-1), x_t2.squeeze(-1), x_flair.squeeze(-1)
            x_fuse = torch.stack([x_t1, x_t1ce, x_t2, x_flair], dim=-1)
            x_fuse = x_fuse.sum(dim=-1)
            # """
            
            x_mamba = self.fuse(x_fuse)

        x_feat = self.features(x)
        if self.p_mode >= 6:
            x = x_feat * (1 - self.mamba_score) + x_mamba * self.mamba_score
        else:
            x = x_feat
        f_x = self.add_ons(x)
        ...
        if self.p_mode >= 6:
            ...
            return f_x, p_map, x, x_mamba, x_feat
    ...
    def forward(self, x, mask=None):
        if self.p_mode >= 6:
            f_x, p_map, x, x_mamba, x_feat = self.conv_features(x, mask)
        ...
        if self.p_mode >= 6 and self.training:
            return logits, min_distances, x, p_map, x_mamba, x_feat
        elif self.p_mode >= 2 and self.training:
    ...
    def push_forward(self, x, mask=None):
        if self.p_mode >= 6:
            f_x, p_map, _, x_mamba, x_feat = self.conv_features(x, mask)
    ...
    def _initialize_weights(self):
        ...
        if self.p_mode >= 6:
            init_conv(self.encoders)
            # init_conv(self.mambas)
            init_conv(self.fuse)

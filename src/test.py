...
if __name__ == '__main__':
    ...
    for i, (I_train, I_test) in enumerate(cv.split(x, y.argmax(1))):
        ...
        for remaining, m_f_x in lcs_test.items():
            for method, lcs_ in lcs_test[remaining].items():
                if not lcs.get(remaining):
                    lcs[remaining] = {}
                if not lcs[remaining].get(method):
                    lcs[remaining][method] = {f'({a}, Th=0.5) {m}': [] for a in ['WT'] for m in ['AP', 'DSC']}
                for metric, lcs__ in lcs_.items():
                    lcs[remaining][method][metric].extend(lcs__.mean(1))
        ...
        print_results("Test", f_x[I_test], y[I_test], m_f_xes_test, lcs_test, n_prototype, iads_test)
